...
async def actor_profile_display(req):
    ...
    tags_chips_html = f'<div style="display:flex; gap:6px; flex-wrap:wrap; margin-top:8px;">{"".join([f"<span style=\\'background:var(--bg3); border:1px solid var(--border); color:var(--muted); font-size:11px; padding:3px 8px; border-radius:4px; font-weight:600;\\'>#{html.escape(t)}</span>" for t in tags_list])}</div>'
    social_html = f'<div style="display:flex; gap:12px; margin-top:12px; flex-wrap:wrap;">{"".join([f"<a href=\\"{html.escape(social[k])}\\" target=\\"_blank\\" style=\\"background:{c}; color:#fff; padding:6px 14px; border-radius:6px; text-decoration:none; font-size:12px; font-weight:700;\\">{l}</a>" for k,c,l in [("instagram","#ff007f","📸 Instagram"),("youtube","#ff0000","📺 YouTube"),("twitter","#1da1f2","🐦 Twitter / X")] if social.get(k)])}</div>'
    ...
    if not gallery_list: gallery_grid_html += '<div style="color:var(--muted); text-align:center; padding:40px;"> 🖼️ Gallery is empty. Upload images to show here.</div>'
    else: gallery_grid_html += f'''<div class="gallery-grid">{"".join([f"""<div class="gallery-item-wrap" onclick="openLightbox('/api/actor/photo?id={actor_id}&gallery_idx={i}')"><img src="/api/actor/photo?id={actor_id}&gallery_idx={i}" class="gallery-item" loading="lazy">{"<button class='gallery-del-btn' onclick='deleteGalleryImage(\\""+actor_id+"\\", "+str(i)+", event)'>🗑️ Delete</button>" if role=='admin' else ""}</div>""" for i in range(len(gallery_list))])}</div>'''
